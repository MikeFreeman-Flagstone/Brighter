...
/// <summary>
/// A decorator for <see cref="KafkaMessageConsumer"/> that retries receiving a message
/// when the underlying consumer returns an empty message (MT_NONE).
/// This is useful in CI environments where Kafka may be slow to deliver messages.
/// </summary>
public class RetryKafkaMessageConsumer : IAmAMessageConsumerSync, IAmAMessageConsumerAsync
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="RetryKafkaMessageConsumer"/> class.
    /// </summary>
    /// <param name="inner">The underlying Kafka message consumer to decorate.</param>
    /// <param name="maxRetries">The maximum number of receive attempts (minimum 1).</param>
    public RetryKafkaMessageConsumer(IAmAMessageConsumerSync inner, int maxRetries = 10)
    {
