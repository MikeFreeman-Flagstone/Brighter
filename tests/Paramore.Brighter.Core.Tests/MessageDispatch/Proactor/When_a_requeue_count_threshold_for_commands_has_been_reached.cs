...
namespace Paramore.Brighter.Core.Tests.MessageDispatch.Proactor
{
    public class MessagePumpCommandRequeueCountThresholdTestsAsync
    {
        ...
        public MessagePumpCommandRequeueCountThresholdTestsAsync()
        {
            ...
            var message1 = new Message(new MessageHeader(Guid.NewGuid().ToString(), _routingKey, MessageType.MT_COMMAND), new MessageBody(JsonSerializer.Serialize((MyCommand)new(), JsonSerialisationOptions.Options)));
            var message2 = new Message(new MessageHeader(Guid.NewGuid().ToString(), _routingKey, MessageType.MT_COMMAND), new MessageBody(JsonSerializer.Serialize((MyCommand)new(), JsonSerialisationOptions.Options)));
        ...
        public async Task When_A_Requeue_Count_Threshold_For_Commands_Has_Been_Reached()
        {
            ...
            await Assert.That(_commandProcessor.Commands[0]).IsEqualTo(CommandType.SendAsync);
            await Assert.That(_commandProcessor.SendCount).IsEqualTo(6);
