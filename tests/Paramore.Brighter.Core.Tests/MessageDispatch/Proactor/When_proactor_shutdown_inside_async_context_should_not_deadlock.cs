...
{
    [Test]
    public async Task When_Proactor_Shuts_Down_Inside_BrighterAsyncContext_Should_Not_Deadlock()
    {
        // Arrange
        ...
        var channel = new AsyncContinuationChannelAsync(new ChannelName("test-deadlock-channel"), routingKey, consumer);
        ...
        var messagePump = new Brighter.ServiceActivator.Proactor(commandProcessor, message => typeof(MyEvent), messageMapperRegistry, new EmptyMessageTransformerFactoryAsync(), new InMemoryRequestContextFactory(), channel);
        messagePump.TimeOut = TimeSpan.FromMilliseconds(5000);
        // Enqueue a message followed by a quit
        var @event = new MyEvent();
        var message = new Message(new MessageHeader(Guid.NewGuid().ToString(), routingKey, MessageType.MT_EVENT), new MessageBody(JsonSerializer.Serialize(@event, JsonSerialisationOptions.Options)));
