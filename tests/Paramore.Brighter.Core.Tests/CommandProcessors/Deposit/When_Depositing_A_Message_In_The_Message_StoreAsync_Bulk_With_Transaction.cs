...
{
    public class CommandProcessorBulkDepositPostWithTransactionTestsAsync
    {
        ...
        public async Task When_depositing_messages_in_the_outbox_with_a_transaction_async()
        {
            ...
            await _commandProcessor.DepositPostAsync(requests, _transactionProvider);
            //assert
            //messages should not be in the outbox
            await Assert.That(_spyOutbox.Messages).DoesNotContain(m => m.Message.Id == _myCommand.Id);
