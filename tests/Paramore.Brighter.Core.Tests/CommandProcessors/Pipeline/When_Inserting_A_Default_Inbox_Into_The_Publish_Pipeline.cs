...
{
    public class CommandProcessorBuildDefaultInboxPublishTests
    {
        ...
        public CommandProcessorBuildDefaultInboxPublishTests()
        {
            ...
            var inboxConfiguration = new InboxConfiguration(_inbox, InboxScope.All, //grab all the events
 onceOnly: true, //only allow once
 actionOnExists: OnceOnlyAction.Throw //throw on duplicates (we should  be the only entry after)
            );
            _commandProcessor = new CommandProcessor(subscriberRegistry, handlerFactory, new InMemoryRequestContextFactory(), new PolicyRegistry { { CommandProcessor.RETRYPOLICY, retryPolicy }, { CommandProcessor.CIRCUITBREAKER, circuitBreakerPolicy } }, new ResiliencePipelineRegistry<string>(), new InMemorySchedulerFactory(), inboxConfiguration: inboxConfiguration);
        }

        [Test]
        public async Task WhenInsertingADefaultInboxIntoThePublishPipeline()
        {
            //act
            ...
            await Assert.That(boxed).IsTrue();
        }
    }
}