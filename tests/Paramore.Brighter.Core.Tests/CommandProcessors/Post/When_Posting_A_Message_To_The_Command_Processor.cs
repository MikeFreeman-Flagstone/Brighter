...
namespace Paramore.Brighter.Core.Tests.CommandProcessors.Post
{
    public class CommandProcessorPostCommandTests
    {
        ...
        public CommandProcessorPostCommandTests()
        {
            ...
            InMemoryMessageProducer messageProducer = new(_internalBus, new Publication() { DataSchema = new Uri("https://goparamore.io/schemas/MyCommand.json"), Source = new Uri("https://goparamore.io"), Subject = "MyCommand", Topic = routingKey, Type = cloudEventsType, ReplyTo = "MyEvent", RequestType = typeof(MyCommand) });
            _expectedMessage = new Message(new MessageHeader(messageId: _myCommand.Id, topic: routingKey, messageType: MessageType.MT_COMMAND, source: messageProducer.Publication.Source, type: messageProducer.Publication.Type, correlationId: _myCommand.CorrelationId, replyTo: messageProducer.Publication.ReplyTo, contentType: new ContentType(MediaTypeNames.Application.Json) { CharSet = CharacterEncoding.UTF8.FromCharacterEncoding() }, partitionKey: _partitionKey, dataSchema: messageProducer.Publication.DataSchema, subject: messageProducer.Publication.Subject, workflowId: _workflowId, jobId: _jobId), new MessageBody(JsonSerializer.Serialize(_myCommand, JsonSerialisationOptions.Options)));
        ...
        [Test]
        public async Task When_Posting_A_Message_To_The_Command_Processor()
        {
