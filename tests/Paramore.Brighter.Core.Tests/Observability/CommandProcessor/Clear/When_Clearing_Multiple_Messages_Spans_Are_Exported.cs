...
public class CommandProcessorMultipleClearObservabilityTests
{
    ...
    public CommandProcessorMultipleClearObservabilityTests()
    {
        ...
        InMemoryMessageProducer messageProducer = new(_internalBus, new Publication { Source = new Uri("http://localhost"), RequestType = typeof(MyEvent), Topic = routingKey, Type = cloudEventsType, });
        var producerRegistry = new ProducerRegistry(new Dictionary<ProducerKey, IAmAMessageProducer> { { new ProducerKey(routingKey, cloudEventsType), messageProducer } });
    ...
    public async Task When_Clearing_A_Message_A_Span_Is_Exported()
    {
        ...
        //assert 
        await Assert.That(_exportedActivities.Count).IsEqualTo(19);
        ...
        await Assert.That(clearActivity.Count()).IsEqualTo(3);
        ...
        await Assert.That(outBoxActivity).HasSingleItem();
        ...
        await Assert.That(producerActivity.Count()).IsEqualTo(3);
    }
}