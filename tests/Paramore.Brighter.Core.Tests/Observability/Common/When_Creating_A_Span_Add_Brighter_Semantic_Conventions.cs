...
public class BrighterTracerSpanTests
{
    ...
    public async Task When_Creating_A_Span_Add_Brighter_Semantic_Conventions()
    {
        ...
        await Assert.That(childSpan.Tags).Contains(t => t.Key == BrighterSemanticConventions.RequestId && (string)t.Value == command.Id.ToString());
        await Assert.That(childSpan.Tags).Contains(t => t.Key == BrighterSemanticConventions.RequestType && (string)t.Value == command.GetType().Name);
        await Assert.That(childSpan.Tags).Contains(t => t.Key == BrighterSemanticConventions.RequestBody && (string)t.Value == System.Text.Json.JsonSerializer.Serialize(command, JsonSerialisationOptions.Options));
        await Assert.That(childSpan.Tags).Contains(t => t.Key == BrighterSemanticConventions.Operation && (string)t.Value == CommandProcessorSpanOperation.Send.ToSpanName());
    }
