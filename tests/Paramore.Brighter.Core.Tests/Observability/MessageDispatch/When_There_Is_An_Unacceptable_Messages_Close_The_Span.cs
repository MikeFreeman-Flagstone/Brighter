...
public class MessagePumpUnacceptableMessageOberservabilityTests
{
    ...
    public MessagePumpUnacceptableMessageOberservabilityTests()
    {
        ...
        _messagePump = new Reactor(commandProcessor, (message) => typeof(MyEvent), messageMapperRegistry, new EmptyMessageTransformerFactory(), new InMemoryRequestContextFactory(), _channel, tracer, instrumentationOptions)
        {
            Channel = _channel,
        ...
        _message = new Message(new MessageHeader(Guid.Empty.ToString(), _routingKey, MessageType.MT_UNACCEPTABLE), new MessageBody(string.Empty));
    ...
    public async Task When_There_Are_No_Messages_Close_The_Span()
    {
        ...
        var emptyMessageActivity = _exportedActivities.FirstOrDefault(a => a.DisplayName == $"{_message.Header.Topic} {MessagePumpSpanOperation.Receive.ToSpanName()}" && a.TagObjects.Any(t => t is { Value: not null, Key: BrighterSemanticConventions.MessageType } && Enum.Parse<MessageType>(t.Value.ToString()) == MessageType.MT_UNACCEPTABLE));
        await Assert.That(emptyMessageActivity).IsNotNull();
        await Assert.That(emptyMessageActivity!.Status).IsEqualTo(ActivityStatusCode.Error);
        await Assert.That(emptyMessageActivity.StatusDescription).Contains($"MessagePump: Failed to parse a message from the incoming message with id {_message.Id} from {_channel.Name}");
    }
}