...
public class MediatorPassingChoiceFlowTests
{
    ...
    public MediatorPassingChoiceFlowTests()
    {
        ...
        var handlerFactory = new SimpleHandlerFactoryAsync((handlerType) => handlerType switch
        {
            ...
            _ when handlerType == typeof(MyOtherCommandHandlerAsync) => new(commandProcessor, _executionLog),
        ...
        var stepOne = new ExclusiveChoice<WorkflowTestData>("Test of Job SequenceStep One", new Specification<WorkflowTestData>(x => x.Bag["MyValue"] as string == "Pass"), () =>
        {
    ...
    public async Task When_running_a_choice_workflow_step()
    {
        ...
        await Assert.That(_stepCompletedThree).IsFalse();
        await Assert.That(_executionLog.Commands).Contains(c => c.Value == "Pass");
        await Assert.That(_executionLog.OtherCommands.Any()).IsFalse();
        await Assert.That(_stepCompletedOne).IsTrue();
    }
}
