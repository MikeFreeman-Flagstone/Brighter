...
public class MediatorPassingChoiceFlowTests
{
    ...
    public async Task When_running_a_choice_workflow_step()
    {
        ...
        await Assert.That(MyCommandHandlerAsync.ReceivedCommands).Contains(c => c.Value == "Pass");
        await Assert.That(MyOtherCommandHandlerAsync.ReceivedCommands.Any()).IsFalse();
        await Assert.That(_stepCompletedOne).IsTrue();
    }
}